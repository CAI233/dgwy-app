...
@Component({
  selector: 'page-shfwa',
  templateUrl: 'sheng-huo-fu-wu-add.html'
})
export class ShengHuoFuWuAddPage {
  files: any = [];
  addParam: any = {
  ...
  formSubmit() {
    if (!this.addParam.content) {
      this.service.dialogs.alert("请输入您的内容", '提示', '确定');
      return false;
    }
    this.addParam.img_path = this.files.join(',');
    this.service.loadingStart();
    this.service.post('/api/life/add/life', this.addParam).then(success => {
      this.service.loadingEnd();
      if (success.code == 600) {
