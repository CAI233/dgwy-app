...
- (NSString *)toolbarTitle
{
    ...
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"照片被选中", @"GMImagePicker", @"%@ 照片被选中" ), @(nImages+nVideos)];
    }
    else if (nImages>1)
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"照片被选中", @"GMImagePicker", @"%@ 照片被选中"), @(nImages)];
    }
    else if (nImages==1)
    {
        return NSLocalizedStringFromTable(@"照片被选中", @"GMImagePicker", @"1 照片被选中" );
    }
    else if (nVideos>1)
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"视频被选中", @"GMImagePicker", @"%@ 视频被选中"), @(nVideos)];
    }
    else if (nVideos==1)
    {
        return NSLocalizedStringFromTable(@" 音频被选中", @"GMImagePicker", @"1 音频被选中");
    }
    else
    {
        return nil;
